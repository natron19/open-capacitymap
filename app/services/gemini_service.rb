...
class GeminiService
  ...
  def self.generate(template:, variables: {}, user: Current.user, response_mime_type: nil)
    new(template:, variables:, user:, response_mime_type:).generate
  end

  def initialize(template:, variables: {}, user:, response_mime_type: nil)
  ...
  def call_gemini(ai_template, rendered_prompt)
    ...
    generation_config[:responseMimeType] = @response_mime_type if @response_mime_type
    # Gemini 2.5 models use thinking tokens by default; disable so maxOutputTokens
    # applies fully to the response rather than being shared with internal reasoning.
    if ai_template.model.include?("gemini-2.5")
